...
def pad_sents(sents, pad_token):
    ...
    """
    sents_padded = []
    ###Padding 技巧，方便进行向量化计算，提升计算效率
    ### YOUR CODE HERE (~6 Lines)
    n_longest = max([len(sentence) for sentence in sents])
    sents_padded = [sentence + [pad_token] * (n_longest - len(sentence)) for sentence in sents ]
    ### END YOUR CODE

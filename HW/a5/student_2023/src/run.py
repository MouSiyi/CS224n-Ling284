...
if args.variant == 'vanilla':
    model = model.GPT(mconf).to(device) # [part c] Make some model here
...
if args.function == 'pretrain':
    ...
    # num_workers=4
    # writer=writer 
    pretrain_conf = trainer.TrainerConfig(max_epochs=650,
                                         batch_size=128,
                                         learning_rate=args.pretrain_lr,
                                         lr_decay=True,
                                         warmup_tokens=512*20,
                                         final_tokens=200*len(pretrain_dataset)*block_size,
                                         num_workers=0, # modified
                                         writer=writer)
    pretrain_trainer = trainer.Trainer(model, pretrain_dataset, None, pretrain_conf)

...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        truncated_doc = doc[:truncate_len]
        truncated_len = len(truncated_doc)

        mask_len = torch.randint(low=1, high=int(2*truncated_len/4), size=(1,))
        mask_idx = torch.randint(low=0, high=int(truncated_len-mask_len), size=(1,))
        prefix = truncated_doc[:mask_idx]
        masked_content = truncated_doc[mask_idx:mask_idx+mask_len]
        suffix = truncated_doc[mask_idx+mask_len:]

        num_pads = self.block_size - truncate_len - 2
        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content + num_pads * self.PAD_CHAR
